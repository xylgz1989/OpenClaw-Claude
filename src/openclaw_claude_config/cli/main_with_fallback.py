"""
Entry point for the OpenClaw Claude Config CLI (with fallback support).
"""
...
from .provider import create_parser as create_provider_parser, execute as execute_provider
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="OpenClaw + Claude Code 统一配置工具 v2.0.1（含智能切换）",
...
示例（新增命令）:
  ...
  %(prog)s provider add --id new --name "New" ...  # 添加提供商
    ...
    create_provider_parser(subparsers)
...
def main():
    ...
    try:
        ...
        if args.command in ["fallback"]:
            return execute_fallback(args)
        elif args.command in ["quota"]:
            return execute_quota(args)
        elif args.command in ["provider"]:
            return execute_provider(args)
            ...
            print(f"\n⚠️  原有命令执行需要完整实现")
            print("新增命令（fallback、quota、provider）已可用")
            return 0
